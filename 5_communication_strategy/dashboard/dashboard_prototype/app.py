...
def main():
    ...
    # Sidebar
    with st.sidebar:
        st.image("https://en.wikipedia.org/wiki/SatNOGS#/media/File:SatNOGS-logo-vertical-black.png", width=200)
        ...
        st.markdown("[🔌 SatNOGS API](https://network.satnogs.org/api/)")
        st.markdown("[📝 Medium Article](https://github.com/ShadiShadab/ELO2-SatNOGs-repo-main/blob/main/5_communication_strategy/Medium_Post.md)")
...
def show_business_rules():
    ...
    api_info = {
        ...
        "SatNOGS Database API": "https://db.satnogs.org/api/",
    ...
    for api_name, api_url in api_info.items():
        st.markdown(f"**{api_name}:** [{api_url}]({api_url})")
...
if observation_time.hour in optimal_hours:
    ...
    st.markdown("---")
    st.markdown("**MIT Emerging Talent Program in Computer and Data Science** | Capstone Project 2025")
